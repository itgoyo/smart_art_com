...
class Order extends BaseController
{
    ...
    public function buyNowSession()
    {
        ...
        ], 'max_buy,state,point_exchange_type,point_exchange,picture,goods_id,goods_name,is_pre_sale');
        ...
        // 如果购买的数量超过库存，则取库存数量
        if ($sku_info['stock'] < $num && $goods_info['is_pre_sale'] == 0) {
            $num = $sku_info['stock'];
        }
        // 获取图片信息
    ...
    public function myOrderList()
    {
        ...
        $where_order['buyer_id'] = $user_info['uid'];
        $where_order['order_status'] = 0;
        $mOrder = new NsOrderModel();
        $orders = $mOrder->where($where_order)->select();
        if(!empty($orders)){
            $order_goods = new OrderGoods();
            $order_goods->checkOrderGoodsValid($orders);
        }
    ...
    public function orderPay()
    {
        ...
        $where['is_valid'] = 0;
        $invalid_goods = $mOrderGoods->where($where)->select();
        if(!empty($invalid_goods)) {
            return $this->error("请先删除无效商品");
        }
    ...
    public function deleteOrderGoods()
    {
        if(request()->isAjax()){
            ...
            if(!empty($m_order_goods)){
                $order_id = $m_order_goods['order_id'];
            }
            $delete_order = $orderService->deleteOrderGoods($order_id, $order_goods_id);
            return AjaxReturn($delete_order);
        }
    }
