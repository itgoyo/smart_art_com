					...
					if(data.length>0){//没登录会返回首页html代码
						for(var i=0;i<data.length;i++){
							...
							if(data[i].point_exchange_type==1){
								str += '<strong>¥'+data[i].price+'+阿特币'+data[i].point_exchange+'</strong>x'+data[i].num+'</div>';
							}else{
				...
				success : function(data){
					...
					if(data.length>0){//没登录会返回首页html代码
						for(var i=0;i<data.length;i++){
							...
							if(data[i].point_exchange_type==1){
								str += '<span class="price">¥'+data[i].price+'+阿特币'+data[i].point_exchange+'x'+data[i].num+'</span>';
							}else{
						...
						$(".total").html("共"+data.length+"种商品，总金额"+total+"元");
					}else{
						//再次查询
