...
class Config extends BaseController
{
    ...
    public function webConfig()
    {
        if (request()->isPost()) {
            ...
            $web_visit = request()->post("web_visit", ''); //是否开启网站登录访问
            $web_reg = request()->post("web_reg", ''); //是否开启网站注册
            $web_check = request()->post('web_check'.'');  // 是否开启采购审核
            ...
            $retval = $this->website->updateWebSite($title, $logo, $web_desc, $key_words, $web_icp, $web_style_pc, $web_style_admin, $visit_pattern, $web_qrcode, $web_url, $web_phone, $web_email, $web_qq, $web_weixin, $web_address, $web_status, $web_visit,$web_reg,$web_check,$wap_status, $third_count, $close_reason, $web_popup_title);
    ...
    public function copyrightinfo()
    {
        ...
        if (request()->isAjax()) {
            ...
            $is_load = 0;
            if ($is_load == 0) {
                $retval = $Config->SetCommonConfig($key, $instance_id, $is_use, $data, $desc);
                return AjaxReturn($retval);
            } else {
                $rs = [
                    'code' => 0,
            ...
            $retval = $Config->getCommonConfig($instance_id,$key,$data);
            $this->assign("info", $retval);
        }
    ...
    public function x360Config()
    {
        ...
        if (request()->isAjax()) {
            ...
            $retval = $Config->getCommonConfig($instance_id,$key,$data);
            $this->assign("config", $retval);
        }
        
        return view($this->style . "Config/x360Config");
    }
    ...
    public function uiConfig()
    {
        ...
        return view($this->style . "Config/uiConfig");
    }
