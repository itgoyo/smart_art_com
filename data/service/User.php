...
class User extends BaseService implements IUser
{
     ...
     */
    public function addOrFindUser($user_info)
    {
        $user_name = $user_info['username'];
        $user = $this->getUserInfoByUsername($user_name);
        if(empty($user)){
            // 用户不存在
            $data['user_name'] = $user_name;
            $data['user_password'] = $user_info['password'];
            $data['user_headimg'] = $user_info['user_headimg'];
            $data['is_system'] = 0;
            $data['is_member'] = 1;
            $data['salt'] = $user_info['salt'];
            $data['sex'] = $user_info['sex'];
            $data['nick_name'] = $user_info['member_name'];
            $uid = $this->user->data([])->allowField(true)->isUpdate(false)->save($data);
            $mMember = new NsMemberModel();
            $member_data['uid'] = $uid;
            $member_data['member_name'] = $user_info['member_name'];
            $member_data['reg_time'] = time();
            $mMember->data([])->allowField(true)->isUpdate(false)->save($member_data);
     ...
     */
     ...
     public function  updateUserInfoByUserid($userid,$password){
         ...
         $data = [
             'user_password' => md5(md5(123456).$salt),
             'salt' => $salt
         ];
...
/**
    ...
    private function initLoginInfo($user_info)
    {
        ...
        Cookie::set($model.'uid',$user_info['uid'],['prefix'=>'smart_','expire'=>3600*24*30]);
        Cookie::set($model.'is_system',$user_info['is_system'],['prefix'=>'smart_','expire'=>3600*24*30]);
        Cookie::set($model.'is_member',$user_info['is_member'],['prefix'=>'smart_','expire'=>3600*24*30]);
        Cookie::set($model.'instance_id',$user_info['instance_id'],['prefix'=>'smart_','expire'=>3600*24*30]);
    ...
    public function login($user_name, $password = '')
    {
        ...
        $user_info = $this->user->getInfo($condition, $field = 'salt,user_password,uid,user_status,user_name,is_system,instance_id, is_member, current_login_ip, current_login_time, current_login_type');
        if (empty($user_info)){
            return LOGIN_FAIL;
        }
        $pwd = md5(md5($password).$user_info['salt']);
        if ($pwd != $user_info['user_password']){
            return USER_ERROR;
        }
        if (! empty($user_info)) {
            if ($user_info['user_status'] == 0) {
                ...
                //登录成功后增加用户的登录次数
                $this->user->where("user_name", "eq", $user_name)
                    ->setInc('login_num', 1);
    ...
    public function add($user_name, $password, $email, $mobile, $is_system, $qq_openid, $qq_info, $wx_openid, $wx_info,$wx_unionid, $is_member, $instance_id = 0)
    {
         ...
         */
        $salt = $this->random(6, '123456789abcdefghijklmnpqrstuvwxyzABCDEFGHIJKLMNPQRSTUVWXYZ');
        $user_password = md5(md5($password).$salt);
        $data = array(
            'user_name' => $user_name,
    ...
    public function ModifyUserPassword($uid, $old_password, $new_password)
    {
        ...
        $user_info = $this->user->getInfo($condition, $field = "salt,user_password,uid");
        if (empty($user_info)){
            return LOGIN_FAIL;
        }
        $pwd = md5(md5($old_password).$user_info['salt']);
        if ($pwd != $user_info['user_password']){
            return USER_ERROR;
        }
        if (! empty($user_info['uid'])) {
            $data = array(
                'user_password' => md5(md5($new_password).$user_info['salt'])
            );
 ...
 /**
    ...
    protected function random($length, $chars = '0123456789') {
        $hash = '';
        $max = strlen($chars) - 1;
        for($i = 0; $i < $length; $i++) {
