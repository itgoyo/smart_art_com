...
class BaseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $uid = Cookie::get($model.'uid','smart_');
        if($uid){
            $uid = Cookie::get($model.'uid','smart_');
            $mUser = new UserModel();
            $m_user =$mUser->where('uid',$uid)->find();

            if(!$m_user){
                //找不到用户 说明 cookie是假的
                Cookie::clear('smart_');
            }else{
                //自动登录密钥一样 把 刚刚查到的id 存session
                Session::set($model.'uid', $m_user['uid']);
                Session::set($model.'is_system', $m_user['is_system']);
                Session::set($model.'is_member', $m_user['is_member']);
                Session::set($model.'instance_id', $m_user['instance_id']);

                Cookie::set($model.'uid',$uid,['prefix'=>'smart_','expire'=>3600*24*30]);
                Cookie::set($model.'is_system',$m_user['is_system'],['prefix'=>'smart_','expire'=>3600*24*30]);
                Cookie::set($model.'is_member',$m_user['is_member'],['prefix'=>'smart_','expire'=>3600*24*30]);
                Cookie::set($model.'instance_id',$m_user['instance_id'],['prefix'=>'smart_','expire'=>3600*24*30]);
    ...
    private function checkLogin()
    {
        $uid = $this->uid;
        if (empty($uid)) {
            $redirect = __URL(__URL__ . "/login/index");
    ...
    public function init()
    {
        ...
        if (empty($web_info['web_popup_title'])){
            $this->assign("web_popup_title", "校360加盟商 电商系统");
        }else{
        ...
        $this->user1 = new User();
        if($this->uid){
            $user_info = $this->user1->getUserInfo();
            if($web_info['web_status'] == 2 && $user_info['is_system'] != 1){
                webClose($web_info['close_reason']);
            }
        }else{
            if($web_info['web_status'] == 2){
        ...
        // 是否开启网站登录访问 1 开启 2 关闭
        if($web_info['web_visit'] == 1){
            $this->checkLogin(); 
        }
