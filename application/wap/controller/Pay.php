...
class Pay extends Controller
{
    ...
    public function wchatPay()
    {
        ...
        //http://nshop.xiao360.com/weixinpay.php
        //echo $red_url;exit;
        $pay = new UnifyPay();
        if (! isWeixin()) {
            // 扫码支付
    ...
    public function wchatUrlBack()
    {
        ...
        if (! empty($postStr)) {
            ...
            if ($postObj->result_code == 'SUCCESS' && $check_sign == 1) {
                ...
                $m_order = $mOrderModel->where('out_trade_no',$postObj->out_trade_no)->find();
                if(!empty($m_order)){
                    $order_id = $m_order['order_id'];
                    $mOrder = new Order();
                    $mOrder->reduceGoodsStock($order_id);
                }
        ...
        if ($verify_result) { // 验证成功
            ...
            if ($trade_status == 'TRADE_FINISHED') {
                ...
                if ($trade_status == 'TRADE_SUCCESS') {
                    ...
                    $mOrderModel = new NsOrderModel();
                    $m_order = $mOrderModel->where('out_trade_no',$out_trade_no)->find();
                    if(!empty($m_order)){
                        $order_id = $m_order['order_id'];
                        $mOrder = new Order();
                        $mOrder->reduceGoodsStock($order_id);
