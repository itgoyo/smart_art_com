...
class GoodsCategory extends BaseService implements IGoodsCategory
{
    ...
    {
        $mGsku = new NsGoodsSkuModel();
        $list = $mGsku->where('goods_id',$goods_id)->select();
        if(!empty($list)){
     ...
     */
    public function addOrEditGoodsCategory($category_id, $category_name, $short_name, $pid, $is_visible, $keywords = '', $description = '', $sort = 0, $category_pic, $category_icon,$attr_id = 0, $attr_name = '')
    {
    ...
    public function getCategoryTreeUseInShopIndex(){
        ...
        $goods_category_one = $goods_category_model->getQuery([
            'level' => 1,
            'is_visible' => 1
        ], 'category_id, category_name,short_name,pid,category_pic,category_icon,sort,attr_name', 'sort');
