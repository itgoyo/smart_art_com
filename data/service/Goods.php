...
class Goods extends BaseService implements IGoods
{
    ...
    public function addTemporary($data)
    {
        ...
        //$stock = $goods['stock'] ? $goods['stock'] : 0;
        //$temporary_stock = $goods['temporary_stock'] ? $goods['temporary_stock'] : 0;
        ...
        try{
            ...
            //更新商品表
            $goods_data['is_temporary'] = 1;
            //$goods_data['stock'] = $stock - $num;
            //$goods_data['temporary_stock'] = $temporary_stock + $num;
            $where_goods['goods_id'] = $data['goods_id'];
            $ret = $mGoods->data([])->isUpdate(true)->save($goods_data,$where_goods);
            if(false === $ret){
                $this->goods->rollback();
                return false;
            }
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price, $cost_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $reduce_type,$is_pre_sale,$is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values)
    {
        $error = 0;
    ...
    public function ModifyGoodsPay($condition)
    {
        $data = array(
            "reduce_type" => 1,
            'update_time' => time()
    ...
    public function ModifyGoodsOrder($condition)
    {
        $data = array(
            "reduce_type" => 2,
            'update_time' => time()
    ...
    public function ModifySalePay($condition)
    {
        $data = array(
            "is_pre_sale" => 1,
            'update_time' => time()
    ...
    public function ModifySaleOrder($condition)
    {
        $data = array(
            "is_pre_sale" => 0,
            'update_time' => time()
    ...
    public function deleteGoods($goods_id)
    {
        ...
        try {
            ...
            // 将商品信息添加到商品回收库中
            $ret = $this->addGoodsDeleted($goods_id);
            //print_r($ret);exit;
            $condition = array(
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        foreach ($goods_sku_detail as $per_goods_sku) {
            $per_goods_sku['real_sale'] = $this->getGoodsSkuRealSale($per_goods_sku['goods_id'],$per_goods_sku['sku_id']);
        }
        ...
        $goods_detail['sales'] = $this->getRealSales($goods_id);
        return $goods_detail;
        // TODO Auto-generated method stub
    }

    protected function getGoodsSkuRealSale($goods_id,$sku_id)
    {
        $mOg = new NsOrderGoodsModel();
        $where['og.refund_status'] = 0;
        $where['og.goods_id'] = $goods_id;
        $where['og.sku_id'] = $sku_id;
        $where['o.pay_time'] = ['gt',0];
        $where['o.order_status'] = ['neq',5];
        $num = $mOg->alias('og')->join('ns_order o','og.order_id = o.order_id','left')->where($where)->sum('og.num');
        return $num;
    }

    protected function getRealSales($goods_id)
    {
        $mOg = new NsOrderGoodsModel();
        $where['og.refund_status'] = 0;
        $where['og.goods_id'] = $goods_id;
    ...
    public function getOrderGoodsRank($condition)
    {
        $order_goods = new NsOrderGoodsModel();
        $order_goods_list = $order_goods->where($condition)
            ->group('goods_id')
            ->field('goods_id,goods_name,sum(goods_money) as total')
            ->order('total desc')
            ->limit(6)
            ->select();
    ...
    public function setGoodsSpotFabulous($shop_id, $uid, $goods_id)
    {
        ...
        if ($retval > 0) {
            $res = $rewardRule->addMemberPointData($shop_id, $uid, $info['click_point'], 19, '点赞赠送阿特币');
        }
        return $retval;
    }
    /**
