...
class Login extends Controller
{
    ...
    public function tklogin()
    {
        ...
        if(isset($input['token'])){
            $url = 'http://pro.xiao360.com/api/open/get_userinfo_by_token';
            $post_data['token'] = $input['token'];
            $res = $this->request_post($url, $post_data);
            $res = json_decode($res,true);
            $info = $res['data'];

            $xiao360_host = $info['database_host'];
            $store_host = xconfig('X360','host');
            if($xiao360_host != $store_host){
            ...
            if(!empty($info)) {
                ...
                $mUser = new User();
                $user = $mUser->addOrFindUser($info);
                $ret = $mUser->UidLogin($user['uid']);
                if ($ret == 1) {
