...
$(function(){
	...
	$(".btn-jiesuan").click(function(){
		if(validationOrder()){
			...
			var card_id = 0;
			if($("#card_id").val() != undefined){
				card_id = $('#card_id').val() == '' ? 0 : $('#card_id').val();
			}
			var integral = $("#count_point_exchange").val() == ""? 0 : $("#count_point_exchange").val();//阿特币
...
function validationCard()
{
	if($('#card').val() != undefined) {
        ...
        $.ajax({
            type : "post",
            url :  __URL(SHOPMAIN + "/member/validecard"),
...
function calculateTotalAmount(){
	...
	// 使用阿特礼品卡
	if($("#card_money").text() != undefined){
        var card_money = $("#card_money").text() == "" ? 0:parseFloat($("#card_money").text());
        if(card_money>0){
            money -= card_money;
        }
	}
