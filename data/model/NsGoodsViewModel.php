...
class NsGoodsViewModel extends BaseModel {
    ...
    public function getGoodsViewList($page_index, $page_size, $condition, $order){
        ...
        if(!empty($list)){
            foreach ($list['data'] as $item) {
                ...
                $item['min_promotion_price'] = $mGk->where('goods_id',$item['goods_id'])->min('promote_price');
                $item['max_promotion_price'] = $mGk->where('goods_id',$item['goods_id'])->max('promote_price');

                $item['real_sales'] = $this->getRealSales($item['goods_id']);
            }
        }
...

    {
        ...
        ->field('ng.goods_id,ng.reduce_type, ng.goods_name, ng.shop_id, ng.category_id, ng.brand_id, ng.group_id_array,
            ...
            ng.description, ng.QRcode, ng.code, ng.is_stock_visible, ng.is_temporary,ng.temporary_stock,ng.is_hot, ng.is_recommend, 
