...
class Order extends BaseService implements IOrder
{
        ...
        if (! empty($order_list['data'])) {
            foreach ($order_list['data'] as $k => $v) {
                ...
                foreach ($order_item_list as $key_item => $v_item) {
                    ...
                    $m_gk = NsGoodsSkuModel::get($v_item['sku_id']);
                    $order_item_list[$key_item]['sku_name'] = $m_gk['sku_name'];
     ...
     */
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id,$card_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin = 0)
    {
        $order = new OrderBusiness();
        if ($pay_type == 4) {
        ...
        $retval = $order->orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id,$card_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin);
            // 提交订单 给客发短信邮件
     ...
     */
    public function checkOrderMoney($orderid)
    {
        try{
            $mOrder = new OrderBusiness();
            $ret = $mOrder->checkOrderMoney($orderid);
        }catch (\Exception $e) {
            Log::write($e->getMessage());
        }
        return $ret;
    }
    public function checkRefundMoney($order_goods_id, $check_refund_status)
    {
        try{
            $mOrderGoods = new OrderGoods();
            $ret = $mOrderGoods->checkRefundMoney($order_goods_id, $check_refund_status);
        }catch (\Exception $e) {
            Log::write($e->getMessage());
        }
        return $ret;
    }
    public function addRefuseReason($order_goods_id, $reason)
    {
        try{
            $mOrderGoods = new OrderGoods();
            $ret = $mOrderGoods->addRefuseReason($order_goods_id, $reason);
        }catch (\Exception $e) {
            Log::write($e->getMessage());
        }
        return $ret;
    ...
    public function orderOffLinePay($order_id, $pay_type, $status)
    {
        ...
        if ($new_no) {
            ...
            if ($retval > 0) {
                ...
                $this->reduceGoodsStock($order_id);
     ...
     * @see \data\api\IOrder::orderMoneyAdjust()
     */
    public function orderNumAdjust($order_goods_id_adjust_array)
    {
        // 调整订单
        $order_goods = new OrderGoods();
        $retval = $order_goods->orderGoodsAdjustNum($order_goods_id_adjust_array);
    ...
    public function deleteOrderGoods($order_id,$order_goods_id)
    {
        $mOrderGoods = new NsOrderGoodsModel();
        $mOrder = new NsOrderModel();
        $mOrderPayment = new NsOrderPaymentModel();
        $order_goods = $mOrderGoods->where('order_id',$order_id)->select();
        // 如果订单只有一条订单项目，删除订单
        $m_order = $mOrder->where('order_id',$order_id)->find();
        $m_order_goods = $mOrderGoods->where('order_goods_id',$order_goods_id)->find();
        $m_order_payment = $mOrderPayment->where('out_trade_no',$m_order['out_trade_no'])->find();
        if(count($order_goods) == 1){
            $m_order->delete();
        }else{
            $data_order['pay_money'] = $m_order['pay_money'] - $m_order_goods['goods_money'];
            $data_order['goods_money'] = $m_order['goods_money'] - $m_order_goods['goods_money'];
            $data_order['order_money'] = $m_order['order_money'] - $m_order_goods['goods_money'];
            $where_order['order_id'] = $order_id;
            $mOrder->save($data_order,$where_order);

            $m_order_payment['pay_money'] -= $m_order_goods['goods_money'];
            $m_order_payment->save();
        }
        $m_order_goods->delete();
        return 1;
    }
    ...
    public function getrefundList($page_index = 1, $page_size = 0, $condition = '',$order_goods = '')
    {
        $mOrderGoods = new NsOrderGoodsModel();

        $refundList = $mOrderGoods->pageQuery($page_index,$page_size,$condition,$order_goods,'*');

        if(!empty($refundList['data'])){
            foreach ($refundList['data'] as $item) {
                $mOrderRefundAccountRecord = new NsOrderRefundAccountRecordsModel();
                $item['order_refund_account_record'] = $mOrderRefundAccountRecord->where('order_goods_id',$item['order_goods_id'])->find();
            }
        }

        $refundList['total_nums'] = $mOrderGoods->getSum($condition,'num');
        $refundList['total_money'] = $mOrderGoods->getSum($condition,'refund_real_money');

        return $refundList;
    }


    public function getConfirmOrderGoodsList($page_index = 1, $page_size = 0, $condition = '',$order = '')
    {
        $mOrderGoods = new NsOrderGoodsModel();
        $ordergoodsList = $mOrderGoods->pageQuery($page_index,$page_size,$condition,$order,'*');
        return $ordergoodsList;
    }

    public function getConfirmRefundList($page_index = 1, $page_size = 0, $condition = '',$order_refund_account_records = '')
    {
        $mOrderRefundAccountRecord = new NsOrderRefundAccountRecordsModel();
        $mOrderGoods = new NsOrderGoodsModel();
        $mOrder = new NsOrderModel();

        $refundList = $mOrderRefundAccountRecord->pageQuery($page_index,$page_size,$condition,$order_refund_account_records,'*');

        if(!empty($refundList['data'])){
            foreach ($refundList['data'] as $item) {
                $order_goods = $mOrderGoods->where('order_goods_id',$item['order_goods_id'])->find();
                $item['goods_info'] = $order_goods['goods_name'];
                $order = $mOrder->where('order_id',$order_goods['order_id'])->find();
                $item['user_name'] = $order['user_name'];
            }
        }

        $refundList['refund_money'] = $mOrderRefundAccountRecord->getSum($condition,'refund_money');
        $refundList['refund_num'] = $mOrderRefundAccountRecord->getCount($condition);
    ...
    public function orderGoodsConfirmRefund($order_id, $order_goods_id, $refund_real_money, $refund_balance_money, $refund_way, $refund_remark)
    {
        ...
        if ($order_refund_chai+1<$refund_real_money_ext) {
            ...
            if ($refund['is_success'] == 1) {
                ...
                // 对于待发货的的商品退回库存
                $this->returnGoodsStock($order_goods_id);
                // 计算平台的账户
    ...
    {
        $order_model = new NsOrderModel();
        $order_money = $order_model->where($condition)->sum('order_money');
        $refund_money = $order_model->where($condition)->sum('refund_money');
        $money = $order_money - $refund_money;
        return $money;
    }
    /**
     *
    ...
    public function getShopOrderAccountDetail($shop_id)
    {
        ...
        $date_week_start=date('Y-m-d', strtotime("$sdefaultDate -".($w ? $w - $first : 6).' days'));
        $date_week_end=date('Y-m-d H:i:s',strtotime("$date_week_start +6 days"));

        $account_week = $this->getShopOrderStatics($shop_id, $date_week_start, $date_week_end);
        // 获取月销售统计(30天)
        $date_month_start = date('Y-m-01', time());
        $date_month_end = date("Y-m-t H:i:s", time());
    ...
    public function getShopAccountCount($shop_id,$date_month_start,$date_month_end)
    {
        // 下单金额
        $order_account = new OrderAccount();
        $condition["create_time"] = ['between',[$date_month_start,$date_month_end]];
        $condition['order_status'] = ['not in',[0,5]];
        if ($shop_id != 0) {
    ...
    public function getShopGoodsSalesList($page_index = 1, $page_size = 0, $condition = '', $order = '')
    {
        ...
        if (! empty($condition["order_status"])) {
            ...
            unset($tmp_array['create_time']);
        }
        //$goods_list = $goods_model->pageQuery($page_index, $page_size, $tmp_array, $order, '*');
        $goods_sku_list = $goods_sku_model->pageQuery($page_index,$page_size,$tmp_array,$order,'*');

        // 条件
        if(isset($condition['create_time']) && $condition['create_time'] != ''){
            $order_condition['create_time'] = $condition['create_time'];
        }
        ...
        foreach ($goods_sku_list["data"] as $k => $v) {
            $goods_sku_list["data"][$k]['goods_name'] = $goods_model->where('goods_id',$v['goods_id'])->value('goods_name');
            $data = array();

            $goods_sales_num = $this->getGoodsSalesNum($order_condition, $v["goods_id"],$v['sku_id']);
            $goods_sales_money = $this->getGoodsSalesMoney($order_condition, $v["goods_id"],$v['sku_id']);

            $data["sales_num"] = $goods_sales_num;
            $data["sales_money"] = $goods_sales_money;
            $goods_sku_list["data"][$k]["sales_info"] = $data;
            //$goods_list["data"][$k]["sales_money"] = $goods_sales_money;
        }
        return $goods_sku_list;
    }

    protected function getGoodsSalesNum($condition,$goods_id,$sku_id)
    {
        $mOg = new NsOrderGoodsModel();
        if(!empty($condition['order_status'])) {
            $where['o.order_status'] = $condition['order_status'];
        }
        if(!empty($condition['create_time'])) {
            $where['o.create_time'] = $condition['create_time'];
        }
        if($sku_id){
            $where['og.sku_id'] = $sku_id;
        }
        $where['og.goods_id'] = $goods_id;
        $where['og.refund_status'] = 0;
        $num = $mOg->alias('og')->join('ns_order o','o.order_id = og.order_id','left')->where($where)->sum('og.num');
        return $num;
        //return 100;
    }

    protected function getGoodsSalesMoney($condition,$goods_id,$sku_id)
    {
        $mOg = new NsOrderGoodsModel();
        if(!empty($condition['order_status'])) {
            $where['o.order_status'] = $condition['order_status'];
        ...
        $money = $mOg->alias('og')->join('ns_order o','o.order_id = og.order_id','left')->where($where)->sum('og.goods_money');
        return $money;
        //return 1000;
    }
     ...
     * @return [type]            [description]
     */
    public function getShopRefundSum($condition)
    {
        $mOrderAccount = new OrderAccount();
        $refund_money = $mOrderAccount->getShopRefundSum($condition);
        return $refund_money;
    }
    /**
     * 查询一段时间内 退款数量
     ...
     * @return [type]            [description]
     */
    public function getShopRefundNumSum($condition)
    {
        $mOrderAccount = new OrderAccount();
        $refund_num = $mOrderAccount->getShopRefundNumSum($condition);
        return $refund_num;
    }
    /**
     * ***********************************************店铺账户--Start******************************************************
    ...
    private function returnGoodsStock($order_goods_id)
    {
        $mOg = new NsOrderGoodsModel();
        $m_order_goods = $mOg->where('order_goods_id',$order_goods_id)->find();
        if($m_order_goods['shipping_status'] == 0){
            $sku_id = $m_order_goods['sku_id'];
            $refund_stock = $m_order_goods['num'];
            $m_goods_sku = NsGoodsSkuModel::get($sku_id);
            $m_goods_sku['stock'] += $refund_stock;
            $m_goods_sku->save();

            $goods_id = $m_order_goods['goods_id'];
            $m_goods = NsGoodsModel::get($goods_id);
            $m_goods['stock'] += $refund_stock;
            $m_goods->save();
        }
        return true;
    }
    ...
    public function commentPoint($order_id)
    {
        ...
        if ($retval > 0) {
            // 给总记录表加记录
            $result = $rewardRule->addMemberPointData($shop_id, $uid, $info['comment_point'], 20, '评论赠送阿特币');
        }
    }
    /**
     *
    ...
    public function reduceGoodsStock($order_id)
    {
        $mOrderGoods = new NsOrderGoodsModel();
        $m_order_goods = $mOrderGoods->where('order_id',$order_id)->select();
        foreach ($m_order_goods as $order_goods) {
            $mGoods = new NsGoodsModel();
            $m_goods = $mGoods->where('goods_id',$order_goods['goods_id'])->find();
            $mGoodsSku = new NsGoodsSkuModel();
            $m_goods_sku = $mGoodsSku->where('sku_id',$order_goods['sku_id'])->find();

            if($m_goods['reduce_type'] == 1 && $m_goods['is_pre_sale'] == 0){  // 线上付款成功减库存 预售商品除外
                $data_goods['stock'] = $m_goods['stock'] - $order_goods['num'];
                $where_goods['goods_id'] = $order_goods['goods_id'];
                $mGoods->save($data_goods,$where_goods);

                $data_goods_sku['stock'] = $m_goods_sku['stock'] - $order_goods['num'];
                $where_goods_sku['sku_id'] = $order_goods['sku_id'];
                $mGoodsSku->save($data_goods_sku,$where_goods_sku);
            }
        }
        return true;
    }
    ...
    public function getOrderNoByOutTradeNo($out_trade_no)
    {
        if (! empty($out_trade_no)) {
            ...
            $mOrderGoods = new NsOrderGoodsModel();
            $m_order_goods = $mOrderGoods->where('order_id',$order['order_id'])->select();
            foreach ($m_order_goods as $order_goods) {
                $mGoods = new NsGoodsModel();
                $m_goods = $mGoods->where('goods_id',$order_goods['goods_id'])->find();
                $mGoodsSku = new NsGoodsSkuModel();
                $m_goods_sku = $mGoodsSku->where('sku_id',$order_goods['sku_id'])->find();
                if($m_goods['reduce_type'] == 1){  // 线上付款成功减库存
                    $data_goods['stock'] = $m_goods['stock'] - $order_goods['num'];
                    $where_goods['goods_id'] = $order_goods['goods_id'];
                    $mGoods->save($data_goods,$where_goods);
