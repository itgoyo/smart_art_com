...
class OrderGoods extends BaseService
{
     ...
     * give_point int(2) NOT NULL DEFAULT 0 COMMENT '阿特币数量',
    ...
    {
        //echo 'gggggggggggg';exit;
        $this->order_goods->startTrans();
        try {
            $err = 0;
            // 商品满赠优惠活动
            $goods_mansong = new GoodsMansong();
            $goods_manzeng_array = $goods_mansong->getGoodsManzengList($goods_sku_list);
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $sku_id = $goods_sku[0];
                if(isset($goods_manzeng_array[$sku_id])){
                    $goods_sku[1] += $goods_manzeng_array[$sku_id]['present'];
                }
                ...
                ], 'goods_name,price,goods_type,picture,promotion_type,promote_id,point_exchange_type,give_point,is_pre_sale');
                ...
                // 不是预售商品才提示库存不足，预售商品不提示
                if (($goods_sku_info['stock'] < $goods_sku[1] || $goods_sku[1] <= 0) && $goods_info['is_pre_sale'] == 0) {
                ...
                $m_goods = $mGoods->where('goods_id',$goods_sku_info['goods_id'])->find();

                if($m_goods['reduce_type'] != 1 && $m_goods['is_pre_sale'] != 1) {
    ...
    /**
     * 退款审查
     * @param  [type] $order_goods_id      [description]
     * @param  [type] $check_refund_status [description]
     * @return [type]                      [description]
     */
    public function checkRefundMoney($order_goods_id,$check_refund_status)
    {
        $data['check_refund_status'] = $check_refund_status;
        $condition['order_goods_id'] = $order_goods_id;
        $mOrderGoods = new NsOrderGoodsModel();
        $ret = $mOrderGoods->save($data,$condition);
        return $ret;
    }
    /**
     * 仓库添加拒绝退款原因
     ...
     * @return [type]                 [description]
     */
    public function getRefundReason($order_goods_id)
    {
        $mOrderGoods = new NsOrderGoodsModel();
        $res = $mOrderGoods->getQuery([
            'order_goods_id' => $order_goods_id
        ], "reason", '');
        $reason = "";
    ...
    public function orderGoodsAdjustNum($order_goods_id_adjust_array)
    {
        $this->order_goods->startTrans();
        try {
            $order_goods_id_adjust_array = explode(';', $order_goods_id_adjust_array);
            if (! empty($order_goods_id_adjust_array)) {
                foreach ($order_goods_id_adjust_array as $k => $order_goods_id_adjust) {
                    ...
                    $order_goods_id = $order_goods_adjust_array[0];
                    $adjust_num = $order_goods_adjust_array[1];
    ...
    public function checkOrderGoodsValid($orders)
    {
        foreach ($orders as $order) {
            $m_order_goods = new NsOrderGoodsModel();
            $order_goods = $m_order_goods->where('order_id',$order['order_id'])->select();
            if(!empty($order_goods)){
                foreach ($order_goods as $goods) {
                    $where['goods_id'] = $goods['goods_id'];
                    $where['sku_id'] = $goods['sku_id'];
                    $mGoodsSku = new NsGoodsSkuModel();
                    $m_goods_sku = $mGoodsSku->where($where)->find();
                    $mGoods = new NsGoodsModel();
                    $m_goods = $mGoods->where('goods_id',$goods['goods_id'])->find();
                    if($m_goods_sku['stock'] == 0 && $m_goods['is_pre_sale'] == 0){
                        $goods['is_valid'] = 0;
                        $goods->save();
                    }
                }
