...
class Promotion extends BaseController
{
    ...
    public function manzengList()
    {
        ...
        $child_menu_list = array(
            array(
                'url' => "promotion/manzenglist",
                'menu_name' => "全部",
                "active" => $status == '-1' ? 1 : 0
            ),
            array(
                'url' => "promotion/manzenglist?status=0",
                'menu_name' => "未发布",
                "active" => $status == 0 ? 1 : 0
            ),
            array(
                'url' => "promotion/manzenglist?status=1",
                'menu_name' => "进行中",
                "active" => $status == 1 ? 1 : 0
            ),
            array(
                'url' => "promotion/manzenglist?status=3",
                'menu_name' => "已关闭",
                "active" => $status == 3 ? 1 : 0
            ),
            array(
                'url' => "promotion/manzenglist?status=4",
                'menu_name' => "已结束",
                "active" => $status == 4 ? 1 : 0
            )
        );
        $this->assign('child_menu_list', $child_menu_list);
        return view($this->style . "Promotion/manzengList");
    }
    ...
    public function addMansong()
    {
        ...
        if (request()->isAjax()) {
            ...
            $goods_id_array = request()->post('goods_id_array', '');
            $res = $mansong->addPromotionMansong($mansong_name,$promotion_type, $start_time, $end_time, $shop_id, '', $type, $range_type, $rule, $goods_id_array);
            return AjaxReturn($res);
        } else {
    ...
    public function addManzeng()
    {
        ...
        if (request()->isAjax()) {
            ...
            return view($this->style . "Promotion/addManzeng");
        }
    }
