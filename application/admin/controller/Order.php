...
class Order extends BaseController
{
    ...
    public function orderList()
    {
        if (request()->isAjax()) {
            ...
            $is_refund = request()->post('is_refund',0);
            ...
            if ($order_status != '') {
                ...
                } else if($order_status == 7){  //部分退款
                    $mOg = new NsOrderGoodsModel();
                    $refund_order_ids = $mOg->where('refund_status',1)->column('order_id');
                    $refund_order_ids = array_unique($refund_order_ids);
                    $condition['order_id'] = ['in',$refund_order_ids];
            ...
            if (! empty($user_name)) {
                $condition['receiver_name'] = ['like','%'.$user_name.'%'];
            }

            if(!empty($goods_name)){
                $mOrderGoods = new NsOrderGoodsModel();
                $where_og['goods_name'] = ['like','%'.$goods_name.'%'];
                $order_ids = $mOrderGoods->where($where_og)->column('order_id');
                $condition['order_id'] = ['in',$order_ids];
            }

            if(!empty($card_no)){
                $mCard = new NsCardModel();
                $where_card['card_no'] = $card_no;
                $m_card = $mCard->where($where_card)->find();
                if(!empty($m_card)){
                    $condition['card_id'] = $m_card['card_id'];
                }
            }

            if($is_refund){
                $mOg = new NsOrderGoodsModel();
                $where_og['refund_status'] = ['gt',0];
                $order_ids = $mOg->where($where_og)->column('order_id');
                $condition['order_id'] = ['in',$order_ids];
            }

            if(!empty($user)){
                $condition['user_name'] = ['like','%'.$user.'%'];
            }
            if (! empty($order_no)) {
            ...
            foreach ($list['data'] as $item) {
                $mCard = new NsCardModel();
                $m_card = $mCard->where('card_id',$item['card_id'])->find();
                $item['card_money'] = $m_card['card_money'];
                $item['card_no'] = $m_card['card_no'];
            }
            return $list;
        } else {
...
//     }
    ...
    {
        $mOrder = new OrderService();
        $order_id = request()->post('order_id','');
        $res = $mOrder->checkOrderMoney($order_id);
        return AjaxReturn($res);
    }
    public function checkRefundMoney()
    {
        ...
        $check_refund_status = request()->post('check_refund_status','');
        $res = $mOrder->checkRefundMoney($order_goods_id,$check_refund_status);
        return AjaxReturn($res);
    }
    /**
     * 仓库添加退款 原因
     */
    public function addRefuseReason()
    {
        $mOrder = new OrderService();
        $order_goods_id = request()->post('order_goods_id','');
        $reason = request()->post('reason','');
        $res = $mOrder->addRefuseReason($order_goods_id,$reason);
        return AjaxReturn($res);
    }
    /**
     * 交易关闭
    ...
    public function orderDelivery()
    {
        ...
        //print_r($shipping_type);exit;
        if ($shipping_type == 1) { // 需要物流
        ...
        if(!empty($order_goods_id_array)){
            ...
            foreach ($order_goods_ids as $k => $order_goods_id) {
                ...
                $m_goods = $mGoods->where('goods_id',$goods_id)->find();
                if($m_goods['is_pre_sale'] == 1){  //预售商品
                    $m_goods['stock'] -= $m_order_goods['num'];
                    $m_goods->save();

                    $mGoodsSku = new NsGoodsSkuModel();
                    $m_goods_sku = $mGoodsSku->where('sku_id',$sku_id)->find();
                    $m_goods_sku['stock'] -= $m_order_goods['num'];
                    $m_goods_sku->save();
                }
            }
        }
        ...
        $mStock->addOutStockLog($order_goods_id_array,$uid,$user_name);
     ...
     * 订单数量调整
     */
    public function orderAdjustNum()
    {
        //$order_id = request()->post('order_id', '');
        $order_goods_id_adjust_array = request()->post('order_goods_id_adjust_array', '');
        $order_service = new OrderService();
        $res = $order_service->orderNumAdjust($order_goods_id_adjust_array);
        return AjaxReturn($res);
    }
    ...
    public function orderGoodsConfirmRecieve()
    {
        ...
        if($isStorage == 1){
            $mStock = new Stock();
            $mStock->addStockLog($storage_num,$goods_id,$sku_id,$order_goods_id,$uid,$user_name);
        }
    ...
    public function getRefuseReason()
    {
        ...
        $order_goods_id = request()->post('order_goods_id', '');
        $ret = $mOrderGoods->getRefundReason($order_goods_id);
        return $ret;
    }
    /**
     * 获取修改收货地址的信息
    ...
    public function orderDataExcel()
    {
        ...
        $list = $list["data"];
        foreach ($list as $k => $v) {
            $list[$k]['pay_money'] = $list[$k]['card_money'] > 0 ? $list[$k]['pay_money'] + $list[$k]['card_money'] : $list[$k]['pay_money'] + $list[$k]['user_money'];
