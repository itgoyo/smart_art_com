...
function createTable(){
	...
	for(var i = 0; i < $sku_array.length; i ++){
		...
		html +='<td class="text-right">'+$temp_Obj[child_id_string]["real_sale"]+'</td>';
...
function synchroSkuValueData(){
	...
	for(var i = 0; i < $sku_array.length ; i++ ){
		...
		if(sku_str == ""){
			sku_str = sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"]+"¦"+value_array["real_sale"];
		}else{
			sku_str +="§"+sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"]+"¦"+value_array["real_sale"];
		}
	}
...
function updateTempObjData(sku_data){
	...
	if($specObj.length > 0){
		$.each(sku_data,function(c,v){
            ...
            $temp_Obj[v.attr_value_items]["real_sale"] = v.real_sale;
