								...
								html += '<td width="80%" style="border:1px solid #E9E9E9;">';
									switch(parseInt(curr.type)){
										...
										case 3:
											...
											console.log(curr.attr_value_id);
											break;
									}
								html += '</td>';
							html += '</tr>';
...
function SubmitProductInfo1(type, ADMIN_MAIN,SHOP_MAIN) {
    img_id_arr = "";// 商品主图
    //var leftCss = new Array();
    // 第一个循环对商品图片进行排序
//	for (var j = 0; j < 5; j++) {
...
//		return;
        $.ajax({
            ...
            dateType : "json",
            success : function(res) {
                var url = __URL(ADMIN_MAIN + "/goods/goodscosts");
                var goodsId = parseInt($("#goodsId").val());
                ...
                if (res != null) {
                    ...
                    showMessage('success', "商品发布成功",__URL(ADMIN_MAIN +'/goods/goodscost'));
                } else {
                    showMessage('error', "商品发布失败",url);
                    flag = false;
                    $("#lastPage,#btnSave,#btnSave2").removeAttr("disabled")
...
function PackageProductInfo() {
	...
	productViewObj.reduce_type = $('input[name="reduce_type"]:checked ').val(); //扣减库存方式
	productViewObj.is_pre_sale = $('input[name="is_pre_sale"]:checked ').val(); // 销售方式
