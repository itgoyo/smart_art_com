...
class Order extends BaseService
{
    ...
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id, $card_id,$user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin)
    {
        ...
        try {
            ...
            $where_card['card_id'] = $card_id;
            $where_card['is_destroy'] = 0;
            $where_card['is_use'] = 0;
            $where_card['is_issue'] = 1;
            $where_card['expire_date'] = ['gt',time()];
            $m_card = $mCard->where($where_card)->find();
            if(!empty($m_card)){
                $card_money = $m_card['card_money'];
                $m_card['is_use'] = 1;
                $m_card->save();
            }

            $pay_money = $order_money - $user_money - $platform_money - $card_money;
            if ($pay_money <= 0) {
                $pay_money = 0;
                $order_status = 0;
                $pay_status = 0;
            ...
            $data_order = array(
                ...
                'point' => $point, // int(11) NOT NULL COMMENT '订单消耗阿特币',
                'point_money' => $point_money, // decimal(10, 2) NOT NULL COMMENT '订单消耗阿特币抵多少钱',
                ...
                'give_point' => $give_point, // int(11) NOT NULL COMMENT '订单赠送阿特币',
            ...
            $real_uid = 0;
            if(!empty($m_order)){
                $real_uid = $m_order['buyer_id'];
            }
            $this->addOrderAction($order_id, $real_uid, '创建订单');
    ...
    public function OrderTakeDelivery($orderid)
    {
        ...
        try {
            ...
            $mOrder = new NsOrderModel();
            $m_order = $mOrder->where('order_id',$orderid)->find();
            $real_uid = 0;
            if(!empty($m_order)){
                $real_uid = $m_order['buyer_id'];
            }
            $this->addOrderAction($orderid, $real_uid, '订单收货');
            // 判断是否需要在本阶段赠送阿特币
    ...
    public function orderComplete($orderid)
    {
        ...
        try {
            ...
            $this->addOrderAction($orderid, $real_uid, '交易完成');
    ...
    private function calculateOrderGivePoint($order_id)
    {
        ...
        $member_account->addMemberAccountData($point['shop_id'], 1, $point['buyer_id'], 1, $point['give_point'], 1, $order_id, '订单商品赠送阿特币');
    }
    /**
     * 订单完成后统计满减送赠送
     *
            ...
            foreach ($list as $k => $v) {
                ...
                if (! empty($mansong_data)) {
                    ...
                    if ($mansong_data['give_point'] != 0) {
                        $member_account = new MemberAccount();
                        $member_account->addMemberAccountData($order_info['shop_id'], 1, $order_info['buyer_id'], 1, $mansong_data['give_point'], 1, $order_id, '订单满减送赠送阿特币');
                    }
                    // 满减送赠送优惠券
    ...
    public function orderClose($orderid)
    {
        ...
        try {
            ...
            if ($order_info['point'] > 0) {
                $account_flow->addMemberAccountData($order_info['shop_id'], 1, $order_info['buyer_id'], 1, $order_info['point'], 2, $orderid, '订单关闭返还阿特币');
            }
            ...
            foreach ($order_goods_list as $k => $v) {
                ...
                $m_goods = $mGoods->where('goods_id',$v['goods_id'])->find();
                // 退货返回库存
                if ($return_stock == 1 && $m_goods['reduce_type'] == 2) {
            ...
            $this->addOrderAction($orderid, $real_uid, '交易关闭');
            $this->order->commit();
            return 1;
        } catch (\Exception $e) {
    ...
    public function orderGoodsRefundFinish($order_id)
    {
        ...
        try {
            ...
            $refunding_count = $order_goods_model->where("order_id=$order_id AND refund_status=1")->count();
            $refunded_count = $order_goods_model->where("order_id=$order_id AND refund_status=1")->count();
            $shipping_status = $orderInfo->shipping_status;
            $all_refund = 0;

            if($refunding_count > 0){
                $orderInfo->order_status = OrderStatus::getOrderCommonStatus()[7]['status_id']; // 退款中
            }else if ($refunded_count == $total_count  ) {
                $all_refund = 1;
    ...
    public function orderAdjustMoney($order_id, $goods_money, $shipping_fee)
    {
        ...
        try {
            ...
            $this->addOrderAction($order_id, $real_uid, '调整金额');
            $this->order->commit();
            return $retval;
        } catch (\Exception $e) {
            $this->order->rollback();
    ...
    public function pickupOrder($order_id, $buyer_name, $buyer_phone, $remark)
    {
        ...
        try {
            ...
            $this->addOrderAction($order_id, $real_uid, '订单提货' . '提货人：' . $buyer_name . ' ' . $buyer_phone);
            // 记录提货信息
            $order_pickup_model = new NsOrderPickupModel();
            $data_pickup = array(
    ...
    public function giveGoodsOrderPoint($order_id, $type)
    {
        ...
        $order_info = $order_model->getInfo([
            "order_id" => $order_id
        ], "give_point_type,shop_id,buyer_id");
